...
const jwt = require("jsonwebtoken"); // 실제로 사용할 때는 인증 폴더를 하나 만들어서 true,false를 반환하는 함수를 만들어준다.
const SECRET = "abcd";
...
const edit_profile = async (req, res) => {
  ...
  // split() : ()안의 문자를 기준으로 문자열로 잘라내기한 후 배열을 반환
  const [bearer, token] = req.headers.authorization.split(" ");
  if (bearer === "Bearer") {
    try {
      const result = jwt.verify(token, SECRET);
      console.log(result);
      const returnValue = awaitUser.findOne({ where: { id: result.id } });
      if (returnValue) {
        const hash = await bcryptPassword(pw);
