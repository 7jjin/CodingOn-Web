...
//router
app.get("/", (req, res) => {
  console.log(process.env.NAME); // env로 들어간 데이터는 문자열 형식으로 나온다.
...
const createPbkdf = (password) => {
  //pbkdf2 함수(비밀번호, 솔트, 반복횟수, 키의 길이, 알고리즘) 으로 생성이되고
  // 반환되는 값은 Buffer값
...
const cipherEncrypt = (word) => {
  ...
  let encryptedData = cipher.update(word, "utf-8", "base64"); // 암호화할 데이터 처리 (암호화 할 값,입력 인코딩, 출력 인코딩)
...
const decipher = (encryptedData) => {
  ...
  let decryptedData = decipher.update(encryptedData, "base64", "utf-8"); // 복호화라서 인코딩 방식과 출력 인코딩을 바꿔써주면 된다.
...
const saltNumber = 10; //  정수만 가능 (10,11 이 적당)
// 암호화
